...
class distance_heuristic : public boost::astar_heuristic< PathFinder::graph_t, float >
{
    // typedef typename graph_traits< graph_t >::vertex_descriptor Vertex;
	public:
    	distance_heuristic(PathFinder l, PathFinder::vertex_descriptor g) : m_location(l), m_goal(g) {}
    	float operator()(PathFinder::vertex_descriptor u)
    	{
			osg::Vec3 d_vec = m_location.getCoordFromDescriptor(m_goal) - m_location.getCoordFromDescriptor(u);
			float d = d_vec.length();
        	return d;
    	}

private:
	PathFinder m_location;
    PathFinder::vertex_descriptor m_goal;
};
...
void PathFinder::compute(vertex_descriptor start_, vertex_descriptor end_, bool useHeuristic) {
	...
	if (useHeuristic) {
		...
		distance_heuristic heuristic(*this, end);
		boost::astar_search()
