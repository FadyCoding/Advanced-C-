...
// TODO TD2
	auto MyCommand = std::make_shared<NewCommand>(PathFinderCommand::PRIORITY_MAKE_OBSTACLE_PERMANENT, center, size);
	gpf.push(MyCommand);

	return graphPoint;
}
void NewCommand::onPathFinderReady(PathFinder & pf){
	pf.addSizedObstacle(center, size, false);
}

bool NewCommand::onPathFinderModified(GamePathFinder * gpf){
	return false;
}
