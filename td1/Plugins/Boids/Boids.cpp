...
public:
	...
	virtual Target * newTarget(const osg::Vec3 pos, float multiplier, float radius) {
		...
		auto new_target = std::make_shared<Target>(pos, multiplier, radius);
		listTargets.push_back(new_target);

		return new_target.get();
	}
	virtual void deleteTarget(Target * t) {
		// TODO TD1 - See listTargets global
		//takes the ptr out of the shared_ptr list (item is shared ptr)
		listTargets.remove_if([t](auto& item){
			return item.get() == t;
		});
	}
