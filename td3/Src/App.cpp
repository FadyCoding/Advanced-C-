...
class UpdateNodeCallBack : public osg::NodeCallback {

	public:
		
		virtual void operator()(osg::Node* node, osg::NodeVisitor* nv) {
			assertm(node != nullptr, "Node is null");

			double currentTime = nv->getFrameStamp()->getReferenceTime();
			double deltaTime = currentTime - lastFrameTime;

			osg::Vec3 translation = osg::Vec3(50.0f * deltaTime, 0, 0);
			// Update the node using the callback
			auto mat = boost::polymorphic_downcast<osg::MatrixTransform*>(node);
			mat->postMult(
...
osg::ref_ptr<osg::MatrixTransform> makeCloud(const osg::BoundingBox & mapBounds) {
	...
	// TODO TD3
	// Create a cloud animation horizontally
	matCloud->setUpdateCallback(new UpdateNodeCallBack);
...
void App::unLoad() {
	...
	pickNodePath.clear();
	pimpl->viewer.setSceneData(nullptr);
