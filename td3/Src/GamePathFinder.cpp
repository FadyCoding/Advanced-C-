...
// TODO TD2
PathFinderMakeObstaclePermanentCommand::PathFinderMakeObstaclePermanentCommand(const osg::Vec3 & center, float size) :
	PathFinderCommand(PRIORITY_MAKE_OBSTACLE_PERMANENT), center(center), size(size)
{
}

// TODO TD2
bool PathFinderMakeObstaclePermanentCommand::onPathFinderModified(GamePathFinder *) { return false; }

// TODO TD2
void PathFinderMakeObstaclePermanentCommand::onPathFinderReady(PathFinder & pf) {
	pf.addSizedObstacle(center, size, false);		// Permanent obstacle
	// Don't call triggerGraphModified(): it's impossible since it's in the GamePathFinder, and it is useless because it was done when putting the temporary obstacle.
}
...
// TODO TD2
	// Add a command for permanent obstacle
	gpf.push(std::make_shared<PathFinderMakeObstaclePermanentCommand>(center, size));
