...
/// A basic heuristic for A*.
///\todo Add a penality if there's an obstacle on the way.
template <class Graph, class Type>
class DistanceHeuristic : public boost::astar_heuristic<Graph, Type> {
public:
	DistanceHeuristic(PathFinder & pf, const osg::Vec3 & goal, WeightGetter & wg) :
		goal(goal), m_pf(pf), wg(wg)
	{
	}

	typedef typename boost::graph_traits <Graph>::vertex_descriptor vertex_descriptor;
	Type operator()(vertex_descriptor u) {
		osg::Vec3 start = m_pf.getCoordFromDescriptor(u);		// Current vertex
		return wg(start - goal);
	}
...
/// Exception for A* termination.
struct AStarFoundGoal {};

/// Visitor that terminates the A* when we find the goal.
template <class Vertex>
class AStarGoalVisitor : public boost::default_astar_visitor {
public:
	AStarGoalVisitor(Vertex goal) : m_goal(goal) {}
	template <class Graph>
	void examine_vertex(Vertex u, Graph& g) { if (u == m_goal) throw AStarFoundGoal(); }
private:
	Vertex m_goal;
};
...
useHeuristic = false;		// Force
	if (useHeuristic) {
		...
		bool found = false;
		try {
			boost::astar_search(g, start, DistanceHeuristic<graph_t, float>(*this, getCoordFromDescriptor(end), *wg), boost::predecessor_map(&p[0]).distance_map(&d[0]).visitor(AStarGoalVisitor<vertex_descriptor>(end)) );
		}
		catch (AStarFoundGoal & ) {
			// Path found
			found = true;
		}
		if (!found) cleanResult();
	} else {
		// Use exact algorithm
