...
/// Adds adds asynchroneously a permanent sized obstacle.
///\warning This does \b NOT call GamePathFinder::triggerGraphModified() (and this is intentional).
///\version Only sized obstacles with vector center are supported at the moment.
class PathFinderMakeObstaclePermanentCommand : public PathFinderCommand {
public:
	PathFinderMakeObstaclePermanentCommand(const osg::Vec3 & center, float size);
	virtual void onPathFinderReady(PathFinder & pf);
	virtual bool onPathFinderModified(GamePathFinder *);
