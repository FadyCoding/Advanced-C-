...
public:
	...
	virtual Target * newTarget(const osg::Vec3 pos, float multiplier, float radius) {
		auto t = std::make_shared<Target>(pos, multiplier, radius);
		listTargets.push_back(t);
		return t.get();
	}
	virtual void deleteTarget(Target * t) {
		auto it = std::find_if(listTargets.begin(), listTargets.end(), [t](TargetList::value_type & v) { return v.get() == t; });
		if (it != listTargets.end()) listTargets.erase(it);
	}
...
extern "C" BTD_PLUGIN_EXPORT BoidsUtil & getBoidsUtil() { return BoidsUtilSKD::instance(); }
